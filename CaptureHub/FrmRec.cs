...
namespace Rec
{
	public partial class FrmRec : Form
	{
		...
		protected void FrmRec_Load(object sender, EventArgs e)
		{
			 ...
			 _videosFolder = Environment.GetFolderPath(Environment.SpecialFolder.MyVideos);
		...
		if (sfdVideo.ShowDialog() == DialogResult.OK)
			{
				...
				lblVideoFile.Text = _videoFilePath;	
			}
		}
		...
		private async void btnRec_Click(object sender, EventArgs e)
		{
			try
			{
				...
				{
					MessageBox.Show("Informe o nome do arquivo antes de iniciar a gravação.", "Video File", MessageBoxButtons.OK, MessageBoxIcon.Error);
					return;
				}
				...
				string fileName = $"Gravacao_{DateTime.Now:yyyyMMdd_HHmmss}.mp4";

				string outputFile = Path.Combine(@"C:\temp\", fileName);
